...
if __name__ == "__main__":
    ...
    # Experiment
    parser.add_argument("--policy", default="TD3_ABCR")  # Online policy name
    ...
    for t in range(int(args.max_timesteps)):
        ...
        if done:
            ...
            episode_reward = env.get_normalized_score(episode_reward)
            # Reset environment
            state, done = env.reset(), False
            if last_episode_reward != -1:
                policy.update_alpha(last_episode_reward, episode_reward)
            print(
                f"Total T: {t + 1} Episode Num: {episode_num + 1} Episode T: {episode_timesteps} Reward: {episode_reward:.3f} alpha: {policy.alpha:.4f}")
